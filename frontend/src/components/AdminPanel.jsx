...
export function AdminPanel({ onNavigate }) {
  ...
  const calculateCapKgPerWindow = () => {
    const capTonsPerYear = parseFloat(emissionCap);
    if (Number.isNaN(capTonsPerYear) || capTonsPerYear <= 0) {
      return null;
    }
    // Demo mode: treat the provided tons value as the per-window limit (just convert to kg)
    return Math.round(capTonsPerYear * 1000);
  };
  ...
  const handleApprove = async () => {
    ...
    if (!approveModal?.walletAddress) {
      alert("⚠️ Company wallet address is missing.");
      return;
    }

    if (!REGISTRY_ADDRESS) {
      alert("⚠️ Missing VITE_COMPANY_REGISTRY_ADDRESS in env");
      return;
    }
    ...
    if (!rpcUrl || !adminPrivateKey) {
      alert("⚠️ Missing VITE_SEPOLIA_RPC_URL or VITE_ADMIN_PRIVATE_KEY");
      return;
    }

    const capKgPerWindow = calculateCapKgPerWindow();
    if (!capKgPerWindow) {
      alert(
        "⚠️ Unable to derive cap in kg per 10 minutes. Please enter a numeric tons/year value.",
      );
      return;
    }
    ...
    try {
      ...
      const tx = await registry.recordCompanyAndFund(
        approveModal.id,
        approveModal.companyName || "",
        approveModal.walletAddress,
        `${emissionCap} tons/year`,
        capKgPerWindow,
        {
          value: ethers.parseEther(String(FUND_AMOUNT_ETH)),
        },
      );

      await tx.wait();

      const fundingTxHash = tx.hash;
      ...
      await updateDoc(companyRef, {
        ...
        fundedAmountEth: `${FUND_AMOUNT_ETH} ETH`,
        fundedAt: new Date().toISOString(),
        fundingTxHash,
        chainRecordedTxHash: fundingTxHash,
      });

      alert(
        `✅ ${approveModal.companyName} has been approved with emission cap of ${emissionCap} tons/year`,
      );
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[#0d131c]">
      <Background />
      <div className="relative z-10 w-full h-screen overflow-y-auto pb-12">
        {/* Header */}
        ...
        <div className="max-w-7xl mx-auto px-6 mt-10">
          ...
          <div className="flex gap-3 mb-6">
            {[
              ...
              },
              {
                value: "rejected",
                label: "Rejected",
                count: companies.filter((c) => c.status === "rejected").length,
              },
          ...
          {loading ? (
            ...
            <Card className="border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl shadow-emerald-900/10 rounded-2xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {companies.map((company) => (
                <Card
                  key={company.id}
                  className="border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl shadow-emerald-900/10 rounded-2xl hover:border-emerald-500/30 transition-all"
                >
                  <CardContent className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      ...
                      <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                        ...
                        <p className="text-white font-medium">
                          {company.annualProductionCapacity || "N/A"}
                        </p>
                      </div>
                      <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                        <p className="text-gray-400 mb-1">Emission Points</p>
                        <p className="text-white font-medium">
                          {company.emissionPoints || "N/A"}
                        </p>
                      </div>
                    </div>
        ...
        {/* Approve Modal */}
        {approveModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0d131c]/80 backdrop-blur-md p-4">
            <Card className="w-full max-w-lg border-white/10 bg-white/10 backdrop-blur-2xl shadow-2xl shadow-emerald-900/20 rounded-2xl">
              <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <p className="text-white font-medium">
                        {approveModal.industrySector || "N/A"}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-400">Production Capacity</p>
                      <p className="text-white font-medium">
                        {approveModal.annualProductionCapacity || "N/A"}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-400">Energy Source</p>
                      <p className="text-white font-medium">
                        {approveModal.energySource || "N/A"}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-400">Energy Consumption</p>
                      <p className="text-white font-medium">
                        {approveModal.energyConsumption || "N/A"}
                      </p>
                    </div>
                    <div className="col-span-2">
                      <p className="text-gray-400">Wallet Address (Sepolia)</p>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-300">
                      Set Annual Emission Cap (tons/year){" "}
                    ...
                    <p className="text-xs text-gray-500">
                      Demo mode: this annual value (tons) is used as the
                      10-minute cap, so higher numbers mean stricter allowances.
                    </p>
                  </div>
